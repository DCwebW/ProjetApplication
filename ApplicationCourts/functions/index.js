...
const expo = new Expo();
// Instanciation d'un objet Expo pour envoyer des notifications
// functions.firestore.document , ici est utilisé pour déclencher une fonction
// lorsqu'un document est ajouté dans la collection matchs
...
exports.sendNotificationsOnMatchCreation = functions.firestore
    ...
    .onCreate(async (snap, context) => {
      ...
      try {
        ...
        const terrainDoc = await admin.firestore()
            .doc(`terrains/${terrainId}`).get();
        const terrainData = terrainDoc.data();
        console.log(terrainData);
        if (!terrainData || !Array.isArray(terrainData.terrains_abonnements)) {
          console
              .log("Aucun abonné ou terrain_abonnements n'est pas un tableau");
          return;
        }
        const abonnements = terrainData.terrains_abonnements;
        // Messages de notification
        const messages = abonnements
            .filter((abonnement) => Expo.isExpoPushToken(abonnement.token))
            // Ici on vérifie les token expo
            .map((abonnement) => ({
              to: abonnement.token,
              sound: "default",
              title: "Nouveau Match Organisé",
              body: `Nouveau match organisé sur le terrain ${terrainData.name}`,
              data: {terrainId, matchId: context.params.matchId},
            }));

        const chunks = expo.chunkPushNotifications(messages);
        // Ici on divise les notifications en petits groupes
        // (Expo limite le nombre de requêtes en une seule fois)
